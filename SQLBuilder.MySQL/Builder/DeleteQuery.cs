...
namespace SQLBuilder.MySQL.Builder {
    ...
    public class DeleteQuery : BaseQuery {
        ...
        public DeleteQuery(string Database, string Table) {
        ...
        public override string ToString() {
            ...
            sb.AppendLine(String.Format("DELETE FROM {0}.{1}", this._Database, this._Table));
            if (this._Wheres.Count > 0) {
                sb.AppendLine(String.Format("WHERE\n\t({0})", String.Join(" AND ", this._Wheres.ToArray())));
            }
            return sb.ToString();
        }
        #endregion
